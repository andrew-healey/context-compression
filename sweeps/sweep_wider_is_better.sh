        ...
        for lr in 3e-5 3.5e-5 4e-5 5e-5; do
            for total_batch_size in 61440; do
                ...
                for seed in 1338 1339 1340 1341 1342 1343 1344 1345; do
                    ...
                    TORCHRUN_PORT=$((12345 + RANK))
                    { CUDA_VISIBLE_DEVICES=$DOUBLE_RANK,$DOUBLE_RANK_PLUS_ONE torchrun --nproc_per_node=$GPUS_PER_RUN --master_port $TORCHRUN_PORT -m context_compression.train --group wider_is_better_9 \
                    --log_dir $out_dir \
                    --max_lr $lr \
                    --total_batch_size $total_batch_size \
                    --seq_len 256 \
